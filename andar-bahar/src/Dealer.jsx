...
const Dealer = () => {
  ...
  const win_section = (section_id) => {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      newSocket.onopen = () => {
        console.log(
          "WebSocket reconnected and now sending win section message..."
        );
        const winMessage = {
          action: "win_section",
          section_id: section_id,
  ...
  const addPlayer = (playerName) => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      console.log(`Player ${playerName} added to server.`);
  
      // Update the state to add the player if not already selected
      setCurrentPlayers((prev) => {
        if (!prev.includes(playerName)) {
          console.log(`Adding ${playerName} to currentPlayers.`);
  ...
  const [currentPlayers, setCurrentPlayers] = useState([]); // Tracks currently selected players
    ...
    <div className="min-h-screen bg-[#450A03] ">
      ...
      <div className="flex flex-col md:flex-row justify-between  bg-[url('./assets/wood.png')] shadow-lg border-2 border-yellow-600">
        ...
        <img
          key={index}
          id={`player${index + 1}`}
          src={currentPlayers.includes(player) ? redhat : whitehat} // Update dynamically based on state
          alt={currentPlayers.includes(player) ? "red hat" : "white hat"}
          className="h-16 cursor-pointer"
          onClick={() => addPlayer(player)} // Pass player directly
        />
      ))}
        </div>
          ...
          <div className="flex-col justify-center items-center relative">
            ...
            {showDropdown && (
              <div className="absolute left-1/2 -translate-x-1/2 mt-2 w-48 bg-[#971909] text-yellow-300 border-2 border-[#D6AB5D] shadow-lg p-2 gap-2">
                ...
                <button onClick={() => addPlayer("player1")}>Add Player1</button>
          ...
          {showResetPopup && (
            <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
              <div className="bg-white p-4 rounded shadow-lg text-center">
                <p className="mb-4">
                  Are you sure you want to reset the collections?
                </p>
...
const TopMenu = ({ sectionId }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between  bg-[url('./assets/wood.png')] shadow-lg border-2 border-yellow-600">
      ...
      className="h-16 cursor-pointer"
      onClick={() => addPlayer(`player${index + 1}`)}
    />
      </div>
